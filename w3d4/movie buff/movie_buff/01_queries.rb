...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where('name = \'Harrison Ford\' AND
    ord!=1')
end
...
def directed_by_one_of(them)
  ...
  # .where(actors: { name: them })
    ...
    query = Movie.select(:id, :title)
      .joins('JOIN castings ON director_id = actor_id')
      .joins('JOIN actors As act
    ON actor_id = act.id')
      .where('act.name': them)
      .order('title ASC')
      .order('movies.id ASC')

    query2 = Movie.select(:id, :title)
      .joins(:director)
      .where(actors: { name: them })
      .order('title ASC')
      .order('movies.id ASC')
